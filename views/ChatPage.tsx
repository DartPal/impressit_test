...
import type { TEmailPayload, TEventPayload } from '@type'
...
export const ChatPage = () => {
  ...
  const handleAccept = useCallback(
    (tool: string, toolCallId: string, _payload: TEventPayload | TEmailPayload) => {
      approve(toolCallId)
      addToolResult({
        tool,
        toolCallId,
  ...
  return (
    <div className="flex flex-col h-screen max-w-2xl mx-auto w-full">
      <header className="px-4 py-3 border-b border-white/10 bg-[#212121]">
        <Text size="lg" className="font-semibold text-[#ececec]">
          AI Assistant
        </Text>
        <br />
